import type { OrgData, Section as OrgSection, AffiliatedKeywords as OrgAffiliatedKeywords, ObjectType } from "uniorg";
...
export function makeOrgRecord(
  ...
  const rootKeywords = orgTree.children.filter(
    (el) => el.type === "keyword",
  );

  const collection = rootKeywords.filter(el => el.key === "PROPERTY")
    .find(el => el.value.toUpperCase().startsWith("COLLECTION"))?.value.slice("COLLECTION".length).trim()
...
export function makeOrgPost(
  ...
  const langs = (rootKeywords.find(el => el.key === "LANGUAGE")?.value ?? "").split(/\s+/).filter(l => l);

  const reply = rootKeywords.filter(el => el.key === "PROPERTY")
    .find(el => el.value.toUpperCase().startsWith("REPLY_TO"))?.value.slice("REPLY_TO".length).trim()
  ...
  const embeds = embedItems.map(el => {
    // This is a workaround for a typing issue in uniorg
    const affiliated = (el as { affiliated?: OrgAffiliatedKeywords })?.affiliated || {};
    ...
    switch (ATTR_ATPROTO.type) {
      case "app.bsky.embed.images":
        return el.children.filter(el => el.type === "link").map(el => {
          const { width, height } = {
          ...
          const base = {
            ...
            aspect: (isNaN(width) || isNaN(height)) ? undefined : { width, height }
          } as const;
          if (el.linkType === "attachment")
            return { path: el.path, type: "file", ...base } as const
          else if (el.rawLink.startsWith("cid"))
            return { path: el.rawLink.replace(/^cid:\s*/, ""), type: "cid", ...base } as const;
          else
            return null;
        }).filter(el => el !== null)
...
function getText(textEls: ObjectType[]): { text: string, facets: NonNullable<OrgPost['facets']> } {
  return textEls.reduce(({ text, facets }, el) => {
    if (el.type === "text") {
      ...
      const _text = el.children.length ? getText(el.children).text : el.rawLink;
...
function getAffiliatedCaption(attr: OrgAffiliatedKeywords[string]): string {
...
function getAffiliatedAttrs(attr: OrgAffiliatedKeywords[string]): Record<string, string> {
  const attr_list: string[] = (Array.isArray(attr) ? attr : [attr])
  ...
  for (let i = 0; i < attr_list.length; i += 2) {
    const key = attr_list[i]!;
    const value = attr_list[i + 1];
    if (value) {
